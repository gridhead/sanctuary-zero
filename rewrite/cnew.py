    ...
    async for recvjson in websocket:
        try:
            ...
            if recvdict["operands"] == "CONVEYMG" and recvdict["username"] != username:
                print("[" + obtntime() + "] " + formusnm(recvdict["username"]) + " > " + helper_display.wrap_conversational_text(recvdict["mesgtext"]))
                ...
                print("[" + obtntime() + "] " + formusnm(recvdict["username"]) + " > " + helper_display.wrap_conversational_text("You were removed from the chatroom"))
            elif recvdict["operands"] == "USERLIST":
                print("[" + obtntime() + "] " + formusnm(recvdict["username"]) + " > " + helper_display.wrap_conversational_text("Following users are connected"))
...
async def chk_username_presence(websocket, username, chatroom):
    mesgdict = {
        "username": username,
        "operands": "CHEKUSER",
        "mesgtext": "",
    ...
    await websocket.send(json.dumps(mesgdict))
    async for recvdata in websocket:
        return recvdata 


async def identify_yourself(websocket, username, chatroom):
    mesgdict = {
        "username": username,
        "operands": "IDENTIFY",
        "mesgtext": "",
    ...
    async with websockets.connect(servaddr) as websocket:
        try:
            ...
            if chkUserPresence == "False":
                ...
                await identify_yourself(websocket, username, chatroom)
                await prod
                await cons
                #asyncio.get_event_loop().run_forever()
                jeye = asyncio.get_event_loop()
                jeye.run_forever()
            else:
